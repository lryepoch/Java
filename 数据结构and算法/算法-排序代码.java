...
排序算法的稳定性：通俗地讲就是能保证排序前2个相等的数其在序列的前后位置顺序和排序后它们两个的前后位置顺序相同。
								...
								当现实应用中排序的需求：需要区别先后顺序的时候，就必须用到稳定排序。
...
//堆排序
//算法描述：对于给定的n个记录，初始时把这些记录看作一棵顺序存储的二叉树，然后将其调整为一个大顶堆，然后将
堆的最后一个元素与堆顶元素进行交换后，堆的最后一个元素即为最大记录；接着将前（n-1）个元素重新调整为一个大
...
//快速排序：取枢轴（第一个记录），经一趟排序，记录小于枢轴的，交换到前面；记录大于枢轴的，交换到后面。结果将待排序记录分成两个子表，最后枢轴位置在分界处
//                 的位置。分别对左右子表重复上述过程，直至每一个子表只有一个记录时[也就是low=high时，退出循环]，排序完成。
//完成一趟排序，low和high的位置相等，即为枢轴在此趟过程的最终位置，原表被分为两个子表。在一趟排序过程中，枢轴记录是固定的。
//不稳定排序
//时间复杂度O(nlogn)		【递归树的深度就为 [log2n]+1，对整个数组扫描一遍，做n次比较】
//空间复杂度最好O(logn)
//只适用顺序存储结构
...
class Solution{
	public int partition(int[] a,int low,int high){		//一趟排序，结束后分成两个子表
		...
		while(low<high){											//从表的两端交替向中间扫描，先从右到左
			...
			while(low<high&&a[high]>=x) high--;		//从high 所指位置向前搜索，至多到low+1 位置。将比枢纽元素小的交换到低端  
			a[low]=a[high];
			while(low<high&&a[low]<=x) low++;		//将比枢纽元素大的交换到高端
			a[high]=a[low];
		}
		a[low]=x;							//还回给你
		return low;						//返回分界点的位置
	...
	public void qsort(int[] a,int low,int high){		//递归对子表进行排序
		if(low<high){											//不需要=，当=时只剩一个记录
			int x=partition(a,low,high);						 //将表一分为二
...
//归并排序：基于分治法，将一个无序序列，先分成一个个待排序的子序列，然后将相邻两个子序列进行合并，即治之。最终合并成一个有序序列。
